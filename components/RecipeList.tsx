...
const RecipeList: React.FC<RecipeListProps> = ({
  ...
  const [categoryFilter, setCategoryFilter] = useState<RecipeCategory | "Alla">(
    "Alla"
  );
  ...
  const filteredRecipes = recipes.filter((r) => {
    ...
    const matchesCategory =
      categoryFilter === "Alla" || r.category === categoryFilter;
  ...
  const handleSaveMeta = async (updatedRecipe: Recipe) => {
    ...
    <div className="space-y-6 animate-fadeIn pb-24">
      {selectedRecipe && (
        <section className="space-y-3">
          {metaEditorError && <p className="text-sm text-red-600">{metaEditorError}</p>}
          <RecipeMetaEditor
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2 no-scrollbar">
        {["Alla", ...RECIPE_CATEGORIES].map((cat) => (
          <button
            key={cat}
            onClick={() => setCategoryFilter(cat as RecipeCategory | "Alla")}
      ...
      {isAdding && (
        ...
        >
          <h3 className="text-lg font-bold text-emerald-900">Lägg till ny rätt</h3>
          <div>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              ...
              className="px-6 py-3 text-emerald-600 font-bold"
            >
              Avbryt
            </button>
          </div>
            ...
            >
              <div className="flex-1 min-w-0 pr-4">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[9px] bg-gray-50 text-gray-400 border border-gray-100 px-1 rounded uppercase font-bold">
                    {recipe.category}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] text-gray-400">
                  <span className="truncate italic pr-2">
                    {recipe.source || "Ingen källa"}
                  </span>
                  <span className="whitespace-nowrap">
                    Senast: {formatDate(recipe.lastCooked)}
                  </span>
                </div>
              </div>
              <button
