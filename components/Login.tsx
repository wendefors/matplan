...
const Login: React.FC<LoginProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err?.message ?? "Inloggningen misslyckades.");
      setPassword("");
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl border border-gray-100 animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            className="w-full p-4 bg-gray-50 border-2 rounded-2xl focus:outline-none transition-all border-transparent focus:border-emerald-500"
            placeholder="E-post..."
            autoComplete="email"
          />
